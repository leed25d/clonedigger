 ...
 */
public class EveryRuleJava8BaseListener implements Java8Listener {
	 ...
	 * <p>The default implementation does nothing.</p>
	 */
	@Override public void enterCompilationUnit(Java8Parser.CompilationUnitContext ctx) { this.parents.push(ctx); System.out.println("Entering Compilation Unit"); 
        astNodeAncestors.push(new MyAstNode("COMPILATION_UNIT", ctx.getStart().getLine(), 0, 0));
        this.startNode.addChild(astNodeAncestors.peek());
    }
	/**
	 * {@inheritDoc}
	 *
	 * <p>The default implementation does nothing.</p>
	 */
	@Override public void exitCompilationUnit(Java8Parser.CompilationUnitContext ctx) { System.out.println("Exiting Compilation Unit"); }
	/**
	 * {@inheritDoc}
	 *
	 * <p>The default implementation does nothing.</p>
    ...
    private MyAstNode addNode(String nodeName, int line) {
        MyAstNode node = new MyAstNode(nodeName, line, 0, 0);
        MyAstNode parent = astNodeAncestors.peek();
        parent.addChild(node);
        astNodeAncestors.push(node);
        return node;
    }

    private void addStatementNode(String nodeName, int line) {
        MyAstNode node = addNode(nodeName, line);
	...
	@Override public void enterStatement(Java8Parser.StatementContext ctx) { 
        MyAstNode node = astNodeAncestors.peek();
        ParseTree firstChild = ctx.getChild(0);
        if (firstChild.getChildCount() > 0) {
            skipNext = true;
            ParseTree literal = firstChild.getChild(0);
            if (literal instanceof TerminalNode) {
                Token symbol = ((TerminalNode) literal).getSymbol();
                node.setValue(symbol.getText());
            }
        }
    }
	/**
    ...
    Stack<ParserRuleContext> parents = new Stack<>();
    Stack<MyAstNode> astNodeAncestors = new Stack<>();
    boolean wasEnter = true;
    int depth = 1;
	/**
	 * {@inheritDoc}
	 *
    ...
    @Override public void enterEveryRule(ParserRuleContext ctx) { 
       /* wasEnter = true;
        if (!parents.isEmpty() && parents.peek() == ctx.getParent()) {
            ParserRuleContext parent = parents.peek();
            depth++;
            // System.out.println("Found parent: " + depth + ">>" + parent.getStop().getStartIndex() + "," + parent.getStop().getText() + "," + parent.getStop().getStopIndex() + ":" + Java8Parser.ruleNames[parent.getRuleIndex()]);
        } else {
            if (!parents.isEmpty())
                parents.pop();
        }
        ...
        System.out.println("Entering void enterEveryRule(ParserRuleContext ctx): " + spaces + depth + " >> start:(" + getTokenStr(ctx.getStart()) + ") - stop:(" + getTokenStr(ctx.getStop()) + "): " + Java8Parser.ruleNames[ctx.getRuleIndex()]);  
        */
        
        /*
        List<Integer> tokenIndexes = new ArrayList<>();
        if (ctx.getChildCount() > 0) {
            ParserRuleContext lastChild = ctx.getChild(0);
            for (int i=ctx.getStart().getTokenIndex(); i<lastChild.getStart().getTokenIndex(); i++) {
                tokenIndexes.add(i);
            }
            for (int i=1; i<ctx.getChildCount(); i++) {
                ParserRuleContext currentChild = ctx.getChild(i);
                for (int i=lastChild.getStop().getTokenIndex()+1; i<currentChild.getStart().getTokenIndex(); i++) {
                    tokenIndexes.add(i);
                }
                lastChild = currentChild;
            }
        } else {
            for (int i=ctx.getStart().getTokenIndex(); i<=lastChild.getStop().getTokenIndex(); i++) {
                tokenIndexes.add(i);
            }
        }
    
        for (Integer tokenIndex : tokenIndexes) {
            System.out.print();
        }
        */
        ...
        boolean statementRule = statementRules.contains(ruleName);
        if (statementRule) {
            depth++;
            addStatementNode(ruleName, ctx.getStart().getLine());
        } else {
            boolean interestedRule = interestedRules.contains(ruleName);
            if (interestedRule) {
                depth++;
                addNode(ruleName, ctx.getStart().getLine()); 
            }
            boolean preserveLiteral = preserveLiteralRules.contains(ruleName);
            if (!interestedRule || preserveLiteral) {
                MyAstNode parent = astNodeAncestors.peek();
                for (int i=0; i<ctx.getChildCount(); i++) {
                    ParseTree o = ctx.getChild(i);
                    if ( o instanceof TerminalNode ) {
                        TerminalNode tnode = (TerminalNode)o;
                        Token token = tnode.getSymbol();
                        result += "( " + i + ":" + getTokenStr(token) + " ) ";
                        MyAstNode newNode = new MyAstNode(token.getText(), token.getLine(), token.getStartIndex(), token.getStopIndex());
    ...
    private String getTokenStr(Token token) {
        return token.getLine() + ":" + token.getCharPositionInLine() + "-" + token.getStartIndex() + "," + token.getText() + "," + token.getStopIndex() +
                    " - " + token.getTokenIndex();
    }
	/**
	 * {@inheritDoc}
	...
	@Override public void exitEveryRule(ParserRuleContext ctx) { 
        /*if (!parents.isEmpty() && parents.peek() == ctx) {
            parents.pop();
            depth--;
        }*/
        String ruleName = Java8Parser.ruleNames[ctx.getRuleIndex()];
        boolean statementRule = statementRules.contains(ruleName);
        boolean consumeLiteralRule = interestedRules.contains(ruleName);
        if (statementRule || consumeLiteralRule) {
            depth--;
            astNodeAncestors.pop();
            return;
        }
        /*
        for (int i=0; i<ctx.getChildCount(); i++) {
...
/*     String printStr = "Exiting void exitEveryRule(ParserRuleContext ctx)";
            printStr += ctx.getStart().getLine() + ":" + ctx.getStart().getCharPositionInLine() + " - " + ctx.getStart().getStartIndex() + ":" + ctx.getStart().getText() + "," + ctx.getStop().getStopIndex() + "," + ctx.getStop().getText() + "::" + Java8Parser.ruleNames[ctx.getRuleIndex()];
       wasEnter = false;
